...
# haze-edit - edits a blog post or a page in the hazeblogger repository
...
B<haze-edit> [B<-fpqCPV>] [B<-b> I<directory>] [B<-E> I<editor>] I<id>

B<haze-edit> B<-h>|B<-v>

=head1 DESCRIPTION

B<haze-edit> opens an existing blog post or a page with the specified
...
Tells B<haze-edit> to edit a page or pages.

=item B<-P>, B<--post>

Tells B<haze-edit> to edit a blog post or blog posts. This is the default
option.

=item B<-f>, B<--force>

Tells B<haze-edit> to create an empty source file in case it does not
